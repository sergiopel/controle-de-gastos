...
@section('page-title', 'Editar Receita')
    ...
    <form method="POST" action="{{ route('incomes.update', $income->id) }}">
        ...
        <div class="mb-3 row">
            ...
            <input id="description" type="text" class="form-control @error('description') is-invalid @enderror"
                name="description" value="{{ old('description') ?? $income->description }}" autocomplete="description"
                autofocus>
        ...
        <div class="mb-3 row">
            ...
            <input id="amount" type="text" class="form-control @error('amount') is-invalid @enderror" name="amount"
                value="{{ old('amount') ?? number_format($income->amount, 2, ',', '.') }}" autocomplete="amount" autofocus>
        ...
        <div class="mb-3 row">
            ...
            <select id="category_id" class="form-select @error('category_id') is-invalid @enderror" name="category_id">
                ...
                @foreach ($categories as $category)
                    <option value="{{ $category->id }}" @selected(old('category_id') == $category->id || $category->id == $income->category_id)>{{ $category->name }}</option>
                @endforeach
            </select>
        ...
        <div class="mb-3 row">
            ...
            <input id="date" type="date" class="form-control @error('date') is-invalid @enderror" name="date"
                value="{{ old('date') ?? $income->date }}" autocomplete="date" autofocus>
